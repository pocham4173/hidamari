...
}
{
  ...
  assert.ok(f.visible(notebook),'お薬手帳の入口は安心タブの見える場所にある');
  /* 2026-09-21 理絵さん指示: お薬手帳→お薬情報を追加→電話・連絡先→相談の順にする */
  const medicineInfo=f.document.getElementById('medicine-info-area');
  before(notebook,medicineInfo);before(medicineInfo,contacts);before(contacts,consultation);before(contacts,tag);
