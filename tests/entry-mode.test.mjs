...
function fixture(){
 ...
 const c={document:{getElementById:id=>{if(!elements.has(id))elements.set(id,{textContent:''});return elements.get(id);}},previewStorage:{getItem:k=>data.get(k),setItem:(k,v)=>data.set(k,v)},uid:()=> 'owner',gid:()=> 'home',showPage:p=>pages.push(p),afterConsent:()=>pages.push('after-consent'),refreshHousehold:()=>wait,householdDeleting:false,col:()=>({doc:()=>({update:async()=>{}})}),saveRecoveryPointer:async()=>{},startMode:()=>pages.push('started'),watchHouseholdAccess:()=>{},showHouseholdBlocked:m=>errors.push(m),setTimeout:f=>{timers.push(f);return timers.length;},clearTimeout:()=>{}};
...
{
 const e=fixture();e.data.set('mainicoConsent-a','yes');e.c.pickMode('honnin');assert.equal(e.pages.at(-1),'after-consent');
}
{
 const e=fixture();e.c.previewStorage.getItem=()=>{throw Error('blocked');};e.c.pickMode('honnin');assert.match(e.elements.get('entry-state').textContent,/保存情報は消さず/);assert.equal(e.data.get('mainicoMode'),'kazoku');
}
{
 const e=fixture();const pending=e.c.finishSetup();assert.equal(e.pages[0],'household-status-page');assert.match(e.elements.get('household-status-text').textContent,/確認しています/);assert.equal(e.data.get('mainicoMode'),'kazoku');
...
}
{
 ...
 e.c.gid=()=> 'home';const pending=e.c.finishSetup();assert.equal(e.pages.at(-1),'household-status-page');e.release();await pending;assert.equal(e.pages.at(-1),'started');
}
{
 ...
 e.c.agreeConsent('a');e.c.saveName();assert.equal(e.errors.length,2);assert.equal(e.pages.length,0);
}
